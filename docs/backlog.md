...
摘要：
已落地轉錄/摘要模型拆分、provider routing、OpenRouter 診斷、Gladia pre-recorded transcription provider、Gladia media URL smoke、失敗 transcript recovery，並移除 AI provider 自動 fallback。下一步是補 Gladia local media / mixed provider smoke、手動只重跑摘要 UX、Gemini 逐 chunk inline 轉錄合併，以及長媒體全局摘要整合。
