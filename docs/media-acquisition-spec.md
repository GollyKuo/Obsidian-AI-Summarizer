...
7. Gemini 媒體轉錄 request timeout 應獨立於一般文字摘要 timeout；長音訊 Files API `generateContent` 不應沿用 120 秒短 timeout。
8. fallback 不得改變使用者可見輸出：最終仍產生合併 transcript、`transcript.md`、`subtitles.srt` 與同一套 summary handoff。
