...
如果下載頁另外提供 `Manual-slides.html`，它是方便離線閱讀或投影的新手簡報，不是 Obsidian plugin 必要檔案。一般使用者能正常使用外掛時，不需要下載它，也不需要把它複製到 plugin 資料夾。

### 媒體流程額外需求

如果只使用 `網頁 URL` 或 `文字檔案`，通常不需要額外媒體工具。
...
如果你是手動下載離線簡報 `Manual-slides.html` 來教學或投影，想看到新版教學內容時，請另外下載新版簡報檔。這個 HTML 檔只作為額外文件，不會保存 API key，也不會由設定頁開啟。
...
1. Gemini 轉錄 Model：`gemini-2.5-flash`
2. Gemini 摘要 Model：`gemini-2.5-flash`
3. 若 Gemini 轉錄遇到 429、503、timeout 或 high demand，媒體轉錄可改用 `Gladia` / `default`。
4. 若 Gemini 摘要遇到額度或高負載問題，摘要 Provider 可改用 `Mistral`，Mistral 摘要 Model 先用 `mistral-small-latest`。
...
| `本機媒體` | 本機音訊或影片檔 | 需要 |
| `文字檔案` | 已有文章正文、`.txt`、`.md` 或 `transcript.md`，想直接摘要 | 不需要 |
...
1. 開啟 `AI 摘要器`。
2. 選 `文字檔案`。
3. 輸入 `.md` 或 `.txt` 檔案路徑，或按 `選擇檔案`。
4. 按 `開始摘要`。

這個流程會直接用檔案內容產生筆記。適合知乎、登入牆、付費牆或動態網頁無法擷取時，先在瀏覽器複製正文，存成 `.txt` / `.md` 後再摘要；也適合媒體轉錄已完成、摘要失敗、想手動修正逐字稿，或想改用 Mistral 重新摘要時使用。
...
2. 媒體轉錄若遇到 Gemini 429、503、timeout 或 high demand，可改用 `Gladia` / `default`。
3. Gemini 額度不足時，摘要可改用 Mistral。
4. OpenRouter 是選用，只有切到 OpenRouter 摘要時才需要填 key。

如果轉錄 Provider 選 `Gemini`，外掛會先把媒體抽成音訊並產生單一 AI-ready artifact，再優先用 Gemini Files API 上傳音訊轉錄；Files API 失敗時會回到逐 chunk inline 轉錄。Gemini Files API 的遠端暫存與本機 `delete_temp` / `keep_temp` 是不同層級：本機 retention 只控制你的電腦上的暫存檔，遠端檔案依 Gemini API provider policy 暫存與刪除。
...
4. `網頁 URL`、`媒體 URL`、`本機媒體`、`文字檔案` 是否可執行。

媒體流程失敗時，請先看這一頁。

### 使用說明與獨立 HTML 簡報

設定頁的 `使用說明` 分頁是新手入口，會帶你完成第一次使用與 plugin 更新檢查。

這個分頁提供 `前往 AI 模型` 與 `前往診斷` 按鈕，方便你先填 API key，再檢查媒體工具。它也整理了更新 plugin 的最短步驟：關閉 Obsidian、覆蓋 release 檔案、不要刪掉整個 plugin 資料夾、重新開啟後檢查設定。

使用說明分頁的內容應以外掛內建內容為準，並隨 `main.js` 一起更新。獨立的 `Manual-slides.html` 適合拿來離線瀏覽、課堂投影或分享給尚未安裝外掛的人，但它不會綁進 Obsidian 設定頁。

簡單判斷：

1. 在 Obsidian 設定頁看說明：更新 plugin 即可。
2. 用瀏覽器開 `Manual-slides.html`：自行下載新版 HTML 才會看到新版簡報。
...
| 按鈕 | 功能 |
| --- | --- |
| `網頁 URL` / `媒體 URL` / `本機媒體` / `文字檔案` | 切換輸入來源 |
| `填入範例` | 填入目前來源類型的範例 |
| `選擇檔案` | 選擇本機媒體或文字檔案 |
| `媒體工具：...` | 檢查媒體工具狀態 |
| `診斷` | 前往設定頁的診斷分頁 |
...
媒體轉錄可改用 Gladia。摘要若繼續使用 Gemini，建議模型用 `gemini-2.5-flash`；若仍遇到額度或高負載問題，可改用 Mistral。
...
選 `文字檔案`，指定 `transcript.md`，再按 `開始摘要`。這樣會跳過轉錄，只重跑摘要與筆記輸出。

### 網頁被阻擋或顯示 403

若網站直接阻擋自動擷取，例如部分知乎文章、登入後內容或付費牆，plugin 無法帶入瀏覽器登入狀態。請改用：

1. 在瀏覽器開啟文章。
2. 複製正文。
3. 存成 `.txt` 或 `.md`。
4. 在 AI Summarizer 選 `文字檔案` 摘要。
